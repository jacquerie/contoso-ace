...
class Entity(db.Model):
    ...
    type = db.Column(db.Enum('DATE', 'LOC', 'TIME', name='type_enum'), nullable=False)
...
class Chat(db.Model):
    ...
    messages = db.relationship(
        'Message', backref='chat', lazy='selectin', order_by=Message.timestamp
    )
    ...
    def get_unassigned_chats():
        return (
            Chat.query.filter(Chat.employee_id.is_(None))
...
class Customer(db.Model):
    ...
    chats = db.relationship(
        'Chat', backref='customer', lazy='selectin', order_by=Chat.last_timestamp
    )
...
class Employee(UserMixin, db.Model):
    ...
    chats = db.relationship(
        'Chat', backref='employee', lazy='selectin', order_by=Chat.last_timestamp
    )
